...
| **First real setup** | A small team | Your own warehouse, an owner, a tester | `RUNBOOK.md` | A tested, Claude-ready lane for one question, gated by `READY-CANVAS.md` |
...
| `RUNBOOK.md` | The build order, step 0 to 11, with who, output and "done when" |
...
| `claude/` | `README.md` with Setups A (Project), B (Claude Code: `CLAUDE.md`, Agent Skill, hook) and C (database connector, `CONNECTOR.md`); the Project upload set in `project/`; the Skill, the SQL guard hook and example `.mcp.json` and `settings.json` files. Which file guides and which enforces |
