 ...
 * vorlagen frontmatter, blog manifest, wie-ki lektionen).
 *
...
/** Reads vorlagen slugs + frontmatter without importing the server-only module. */
function readVorlagen() {
  const dir = join(ROOT, "content", "vorlagen");
  return readdirSync(dir)
    .filter((file) => file.endsWith(".md"))
    .map((file) => {
      const { data } = matter(readFileSync(join(dir, file), "utf8"));
      const slug = file.replace(/\.md$/, "");
      for (const field of ["title", "jobToBeDone", "lastReviewed"]) {
        if (typeof data[field] !== "string" || data[field].trim() === "") {
          throw new Error(`content/vorlagen/${file} is missing required frontmatter: ${field}`);
        }
      }
      if (!/^\d{4}-\d{2}-\d{2}$/.test(data.lastReviewed)) {
        throw new Error(`content/vorlagen/${file} has an invalid lastReviewed date`);
      }
      return {
        slug,
        title: data.title,
        description: data.jobToBeDone,
        freshness: `frontmatter lastReviewed ${data.lastReviewed}`,
      };
    })
...
const vorlagenRows = readVorlagen().map((vorlage) =>
  row([
    `${SITE_ORIGIN}/vorlagen/${vorlage.slug}`,
    vorlage.title,
    vorlage.description,
    `content/vorlagen/${vorlage.slug}.md`,
    vorlage.freshness,
    OWNER,
  ]),
);
...
### Vorlagen-Detailseiten (${vorlagenRows.length})

${table(CONTENT_HEADERS, vorlagenRows)}
