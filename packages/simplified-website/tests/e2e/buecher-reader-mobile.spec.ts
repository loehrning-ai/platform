...
/**
 * Books reader on mobile (regression coverage). Pins the 390x844 layout across
 * ALL THREE books, one real middle chapter each: no horizontal overflow,
 * readable prose, reachable + tappable prev/next, a collapsed desktop TOC, and
 * callouts/tables that stay inside the viewport. Assertions target roles +
 * stable manifest titles, never prose, so a content refresh on another branch
 * stays green while a real layout regression fails. (No chapter ships images,
 * so an image-constraint check is intentionally omitted.)
 */

const MOBILE = { width: 390, height: 844 } as const;

// Real middle chapters read from content/books/<book>/manifest.json: each has
// both a previous and next neighbour and contains GFM tables + known callouts.
const CHAPTERS = [
  {
    book: "ki-arbeitsalltag",
    slug: "04_vier_stufen",
    title: "Die vier Stufen der Datenklassifizierung im Detail",
    next: "05_compliance",
    hasCallouts: true,
  },
  {
  ...
  {
    book: "ki-tools-selbststaendige",
    slug: "03_prompting",
    title: "Die KRAFT-Methode: Prompting, das funktioniert",
    next: "04_akquise",
    hasCallouts: true,
  },
] as const;
