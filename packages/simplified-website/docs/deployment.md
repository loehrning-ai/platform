...
| Setting | Value | Why |
|---|---|---|
| Root Directory | `packages/simplified-website` | Bun workspace monorepo; `next` is a dependency of the nested package only. |
...
| Install / Build command | leave default | Vercel detects Bun from `bun.lock`; `bun run build` triggers `prebuild` (`validate-env` + `registry:export`). |
| Node.js version | 22.x or 24.x | `engines: ^22.0.0 \|\| ^24.0.0`, `.nvmrc` 24.x. The prebuild registry export imports a `.ts` module through plain `node` and relies on type stripping (Node >= 22.18). |
