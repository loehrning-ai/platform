/**
 * Middleware tests (accessibility hardening).
 *
 * Asserts that the middleware does NOT inject any ff_journey_v3 feature flag
...
describe("middleware: ff_journey_v3 feature flag removal (accessibility hardening)", () => {
  it("src/middleware.ts does not reference ff_journey_v3", async () => {
    ...
    expect(
      ...
      "src/middleware.ts must not reference ff_journey_v3 (deleted by public-content transition)",
    ).toBe(false);

    expect(
      content.includes("x-ff-journey"),
      "src/middleware.ts must not set x-ff-journey response header",
    ).toBe(false);
  });
