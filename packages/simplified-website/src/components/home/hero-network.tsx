...
export function HeroNetwork({ className, mobile, frozen, latOut, lonOut, stepIdxOut }: HeroNetworkProps) {
  ...
  // viewer sees the full set rather than just the home country.
  const staticGrid = (prefersReduced || mobile) ? buildGrid(BERLIN_LON, BERLIN_LAT) : null;
  // Outlines: open per-arc paths.
  const staticCountry = prefersReduced && !mobile
  ...
  const staticCountryFill = prefersReduced && !mobile
    ? STEP_COUNTRY.flatMap((key) =>
