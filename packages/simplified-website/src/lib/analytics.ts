 ...
 *
 * Custom event helpers below fire via console.debug in dev so that events are
 * always observable without needing a live analytics provider.
 *
...
export function track(event: string, props?: TrackProps): void {
  ...
  // Dev fallback so events are visible without an analytics provider.
