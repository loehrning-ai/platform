 ...
 * - Template frontmatter (content/vorlagen/)
...
describe("Template frontmatter validation (content/vorlagen/)", () => {
  const templateDir = resolve(process.cwd(), "content/vorlagen");
  const mdFiles = walkDir(templateDir, ".md");

  it("at least one template exists", () => {
  ...
  for (const filePath of mdFiles) {
    ...
    it(`${relPath}: has YAML frontmatter`, () => {
      const content = readFileSync(filePath, "utf-8");
      const hasFrontmatter = content.startsWith("---\n") && content.includes("\n---");
      expect(hasFrontmatter, `${relPath} missing YAML frontmatter`).toBe(true);
    });

    it(`${relPath}: has a non-empty title in frontmatter`, () => {
      const content = readFileSync(filePath, "utf-8");
      const titleMatch = content.match(/^title:\s*(.+)/m);
      expect(
        titleMatch?.[1]?.trim().length,
        `${relPath} missing title in frontmatter`,
      ).toBeGreaterThan(0);
    });
  }
});
