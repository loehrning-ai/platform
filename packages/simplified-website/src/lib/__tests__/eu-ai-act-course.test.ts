...
describe("landing page duration", () => {
  ...
  it("landing page uses PT1H30M (96 min = blockMeta sum)", () => {
    const content = readFile("app/eu-ai-act-kurs/page.tsx");
    expect(content).toContain("PT1H30M");
  });

  it("landing page mentions ca. 1,5 Stunden or 96 min", () => {
    const content = readFile("app/eu-ai-act-kurs/page.tsx");
    const hasTimeRef = content.includes("1,5 Stunden") || content.includes("96 min") || content.includes("14–18 Minuten");
