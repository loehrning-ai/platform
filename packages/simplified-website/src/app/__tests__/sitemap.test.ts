...
import { getVorlagenSlugs } from "@/lib/vorlagen";
...
describe("sitemap()", () => {
  ...
  it("returns only public indexable routes", () => {
    ...
    //   + catalog detail pages (buecher + demos + vorlagen + workshops slugs)
  ...
  it("includes public course previews, blog, and legal routes", () => {
    ...
    expect(urls).toContain("https://loehrning.ai/vorlagen");
  ...
  it("includes catalog detail pages for buecher, demos, vorlagen, and workshops (public-content contract)", () => {
    const urls = result.map((e) => e.url);
    ...
    for (const slug of getVorlagenSlugs()) {
      expect(urls).toContain(`https://loehrning.ai/vorlagen/${slug}`);
    }
