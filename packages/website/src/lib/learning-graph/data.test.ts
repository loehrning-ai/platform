...
describe("book node derivation", () => {
  // "ki-arbeitsalltag" and "ki-tools-selbststaendige" are pending re-review
  // and excluded from the public `books` catalog (see lib/books.ts), so they
  // produce no graph node right now — only "ki-landschaft" is published.
  it("produces no node for books pending re-review", () => {
    expect(nodeById("book:ki-arbeitsalltag")).toBeUndefined();
    expect(nodeById("book:ki-tools-selbststaendige")).toBeUndefined();
  });
  ...
  it("levels the published book entry", () => {
