...
const POPULATED_IDS = ["home", "fund", "explore", "clean", "feature", "model", "eval"] as const;
const UNPOPULATED_IDS = DS_CHAPTER_IDS.filter(
  (id) => !(POPULATED_IDS as readonly string[]).includes(id),
);

describe("data-science chapter-component loader map (plan 012 stages 1, 6, 7, 8)", () => {
  it("resolves a real component for every populated id (fund, eval — stage 6; home, explore, clean — stage 7; feature, model — stage 8)", async () => {
