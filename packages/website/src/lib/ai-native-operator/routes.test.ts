...
describe("ai-native-operator route helpers ", () => {
  ...
  it("moduleHref nests the module id directly under the course root, no /kurs segment", () => {
    expect(moduleHref("mindset")).toBe("/kurse/open-source/ai-native-operator/mindset");
    expect(moduleHref("measurement")).toBe("/kurse/open-source/ai-native-operator/measurement");
  });

  it("lessonHref nests moduleId/lessonNumber under the course root", () => {
    expect(lessonHref("mindset", 1)).toBe("/kurse/open-source/ai-native-operator/mindset/1");
