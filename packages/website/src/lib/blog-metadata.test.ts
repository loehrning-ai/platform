...
describe("BLOG_POSTS manifest", () => {
  ...
  it("each post has non-empty required fields", () => {
    for (const post of BLOG_POSTS) {
      ...
      expect(post.titleEn, `titleEn empty on ${post.slug}`).toBeTruthy();
      expect(post.summary, `summary empty on ${post.slug}`).toBeTruthy();
      expect(post.summaryEn, `summaryEn empty on ${post.slug}`).toBeTruthy();
      expect(
        post.datePublished,
        `datePublished empty on ${post.slug}`,
      ).toBeTruthy();
      expect(
        post.dateModified,
        `dateModified empty on ${post.slug}`,
      ).toBeTruthy();
      expect(post.tags.length, `no tags on ${post.slug}`).toBeGreaterThan(0);
      expect(
        post.tagsEn.length,
        `no English tags on ${post.slug}`,
      ).toBeGreaterThan(0);
      expect(
        post.readingTimeMin,
        `readingTimeMin zero on ${post.slug}`,
      ).toBeGreaterThan(0);
      expect(
        post.postNumber,
        `postNumber zero on ${post.slug}`,
      ).toBeGreaterThan(0);
    }
  });
  ...
  it("uses contiguous post numbers starting at one", () => {
    expect(
      BLOG_POSTS.map((post) => post.postNumber).sort((a, b) => a - b),
    ).toEqual(
  ...
  it("dates are valid ISO 8601 YYYY-MM-DD format", () => {
    ...
    for (const post of BLOG_POSTS) {
      expect(post.datePublished, `${post.slug} datePublished not ISO`).toMatch(
        iso,
      );
      expect(post.dateModified, `${post.slug} dateModified not ISO`).toMatch(
        iso,
      );
    }
  });

  it("has no banned commercial phrases in either locale", () => {
    for (const post of BLOG_POSTS) {
      for (const phrase of BANNED_PHRASES) {
        expect(
          post.titleDe,
          `"${phrase}" in titleDe of ${post.slug}`,
        ).not.toContain(phrase);
        expect(
          post.summary,
          `"${phrase}" in summary of ${post.slug}`,
        ).not.toContain(phrase);
        expect(
          post.titleEn,
          `"${phrase}" in titleEn of ${post.slug}`,
        ).not.toContain(phrase);
        expect(
          post.summaryEn,
          `"${phrase}" in summaryEn of ${post.slug}`,
        ).not.toContain(phrase);
      }
    }
  });
...
describe("getPostNumberLabel", () => {
  ...
  it("throws on an unknown slug", () => {
    expect(() => getPostNumberLabel("ki-und-arbeit")).toThrow(
      /unknown blog slug/,
    );
  });
});
