...
const ARCH_NODES_DE: readonly ArchNode[] = [
  {
    id: "client",
    x: 30,
    y: 85,
    ...
    label: "Client",
    color: "#7B8CDE",
    desc: "Browser, mobile App oder interner Dienst, der eine Vorhersage anfordert.",
    fail: "Zeitüberschreitung bei langsamer Inferenz; unbegrenzte Wiederholungen können das System überlasten.",
  },
  {
    id: "lb",
    x: 155,
    ...
    label: "Load Balancer",
    color: "#A78BFA",
    desc: "Verteilt Anfragen auf API-Replikate. Führt Zustandsprüfungen durch und nutzt Circuit Breaker.",
    fail: "Ohne Redundanz ein einzelner Ausfallpunkt; Sticky Sessions verhindern horizontale Skalierung.",
  },
  {
    id: "api1",
    x: 285,
    ...
    label: "REST API ①",
    color: "#64E2B5",
    desc: "Zustandsloser Vorhersageserver. Validiert Eingaben, lädt Merkmale und ruft das Modellartefakt auf.",
    fail: "Speicherleck, wenn das Modellobjekt nicht zwischen Workern geteilt wird.",
  },
  {
    id: "api2",
    x: 285,
    ...
    label: "REST API ②",
    color: "#64E2B5",
    desc: "Replikat von API ①. Skaliert anhand der Warteschlangentiefe oder CPU-Last automatisch.",
    fail: "Versionsabweichung zu API ①: Nach einem rollierenden Deployment ist ein anderes Modellartefakt geladen.",
  },
  {
    id: "fs",
    x: 415,
    ...
    label: "Feature Store",
    color: "#F4C542",
    desc: "Vorberechnete Merkmalsvektoren, nach Entitäts-ID indiziert. Redis oder DynamoDB liefern sie in unter einer Millisekunde.",
    fail: "Veraltete Merkmale: Die Ingestion-Pipeline hängt zurück und das Modell sieht in Produktion die Daten von gestern.",
  },
  {
    id: "registry",
    x: 415,
    ...
    label: "Model Registry",
    color: "#FF9F6B",
    desc: "Speichert Modellartefakte mit Versionskennzeichen, Metriken und Herkunft. Maßgebliche Quelle dafür, welches Modell aktiv ist.",
    fail: "Fehlende Champion/Challenger-Kennzeichnung führt beim Rollback zum falschen Artefakt.",
  },
  {
    id: "monitor",
    x: 285,
    ...
    label: "Monitoring",
    color: "#FF6B80",
    desc: "Erfasst Vorhersageprotokolle, Feature Drift (PSI), p99-Latenz und die Fehlerrate, sobald Ground-Truth-Labels vorliegen.",
    fail: "Alarmmüdigkeit: Zu viele verrauschte Metriken verdecken echten Drift.",
  },
];

const ARCH_ARROWS: readonly {
  x1: number;
  y1: number;
  x2: number;
  y2: number;
...
export function ModelServingArchitecture() {
  ...
  const nodes = locale === "de" ? ARCH_NODES_DE : ARCH_NODES;
    ...
    >
      <div className="ds-architecture-layout">
        <div className="ds-architecture-visual">
          <div
            className="ds-architecture-scroll"
            data-horizontal-scroll
            role="region"
            aria-label={text(
              "Model serving architecture diagram",
              "Diagramm der Architektur für die Modellbereitstellung",
            )}
            tabIndex={0}
          >
            <svg
              className="ds-architecture-diagram"
              viewBox="0 0 560 240"
              role="img"
              aria-label={text(
                "Model serving architecture from client to APIs, stores, registry, and monitoring",
                "Architektur der Modellbereitstellung vom Client über APIs, Speicher und Registry bis zum Monitoring",
              )}
            >
              <defs>
                <marker
              ...
              {nodes.map((n) => (
                <g
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={text(`Inspect ${n.label}`, `${n.label} prüfen`)}
                ...
                >
                  <rect
                    ...
                    rx="6"
                    fill={
                      activeId === n.id ? n.color : "rgba(244,242,236,0.06)"
                    }
                    stroke={n.color}
          ...
          <div className="ds-architecture-scroll-hint" aria-hidden="true">
            {text(
              "Scroll diagram horizontally →",
              "Diagramm horizontal scrollen →",
            )}
          </div>
        </div>
        <div
          className="ds-architecture-detail"
          style={{
            minHeight: 120,
            padding: "14px 16px",
            borderRadius: 8,
        ...
        >
          {node ? (
            <>
              ...
              >
                {text("Common failure", "Typischer Fehler")}
              </div>
              <div
                style={{
                  fontSize: 12.5,
            ...
            >
              {text(
                ...
                "Komponente mit Maus, Tastaturfokus oder Auswahl aktivieren.",
              )}
            </div>
          )}
        </div>
