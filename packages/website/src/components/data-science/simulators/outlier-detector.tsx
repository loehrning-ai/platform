...
const METHODS: readonly { readonly id: Method; readonly label: string; readonly desc: string }[] = [
...
export function OutlierDetector() {
  ...
  const { points, meanX, sdX, meanY, sdY, q1X, q3X, q1Y, q3Y } = useMemo(() => {
    ...
    const points2: Point[] = rawX.map((x, i) => ({ x, y: rawY[i] ?? 0, isOutlier: i >= 44 }));
    return {
      points: points2,
      meanX: meanX2,
      sdX: sdX2,
    ...
    >
      <div className="sim-row">
        ...
        <div className="plot-wrap">
          <svg viewBox={`0 0 ${W} ${H}`}>
            <line x1={px(0)} y1={PADT} x2={px(0)} y2={PADT + plotH} stroke="rgba(244,242,236,0.08)" strokeWidth="1" />
            <line x1={PADL} y1={py(0)} x2={PADL + plotW} y2={py(0)} stroke="rgba(244,242,236,0.08)" strokeWidth="1" />
            {method === "zscore" && (
              <ellipse
                cx={px(meanX)}
                cy={py(meanY)}
            ...
            <text x={PADL + plotW / 2} y={H - 4} textAnchor="middle" fill="rgba(244,242,236,0.3)" fontSize="8">
              Feature X
            </text>
            <text
              x={8}
