...
export function GaltonSim() {
    ...
    const loop = (now: number) => {
      ...
      for (const b of balls) {
        ...
        if (b.y >= POP_TOP - 6) {
          ...
          if (sampleBufRef.current.length >= n) {
            const mean =
              sampleBufRef.current.reduce((acc, v) => acc + v, 0) / sampleBufRef.current.length;
      ...
      for (const m of meanBallsRef.current) {
        ...
        if (u >= 1) {
          ...
          meanStackRef.current[binIdx] = (meanStackRef.current[binIdx] ?? 0) + 1;
        }
      }
  ...
  const seEmpirical =
    totalSamples > 1 && meanOfMeans !== null
      ? Math.sqrt(
          meansRef.current.reduce((a, v) => a + (v - meanOfMeans) ** 2, 0) / (totalSamples - 1),
        )
      : null;
    ...
    >
      <div className="sim-row galton-row">
        <div className="sim-controls">
          <div className="sim-ctrl">
            ...
            <div className="seg">
              {(["bell", "skew", "bimodal"] as const).map((p) => (
                <button key={p} type="button" className={pop === p ? "on" : ""} onClick={() => setPop(p)}>
                  {p}
                </button>
              ))}
            </div>
          ...
          <div className="sim-ctrl">
            <label>
              Drop rate <span className="mono">{rate}/s</span>
            </label>
            <input
              type="range"
              min="2"
          ...
          <div className="sim-stats">
            <div>
              <div className="k">Mean of means</div>
              <div className="v">{meanOfMeans != null ? round(meanOfMeans, 2) : "—"}</div>
            </div>
            <div>
              <div className="k">SE (empirical)</div>
          ...
          <svg viewBox={`0 0 ${W} ${H}`} className="galton-svg">
            ...
            {ballsRef.current.map((b, i) => {
              if (b.landed) {
                ...
                const binBarH = ((b.stackIdx ?? 0) + 1) / popMax * (POP_BOT - POP_TOP - 10);
            ...
            {meanBallsRef.current.map((m, i) => (
              <g key={i}>
                <circle cx={m.x} cy={m.cy ?? m.y} r={4} fill={m.hue} opacity="0.9" />
                <circle cx={m.x} cy={m.cy ?? m.y} r={8} fill="none" stroke={m.hue} strokeWidth="1" opacity="0.4" />
              </g>
            ))}
            ...
            {totalSamples > 20 && seEmpirical !== null && seEmpirical > 0 && meanOfMeans !== null && (
              <path
                d={normalPath(meanOfMeans, seEmpirical, MEAN_TOP, MEAN_BOT, BIN_W, W)}
                fill="none"
                stroke="#5B3EE8"
                strokeWidth="2"
                opacity="0.85"
            ...
            <line x1="0" y1={MEAN_BOT} x2={W} y2={MEAN_BOT} stroke="#A49D9A" strokeWidth="0.8" />
            <line x1="0" y1={POP_BOT} x2={W} y2={POP_BOT} stroke="#A49D9A" strokeWidth="0.8" />
          </svg>
        </div>
      </div>
    </Panel>
