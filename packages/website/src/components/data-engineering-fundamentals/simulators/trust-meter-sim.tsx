...
export const CHECKS: readonly CheckDef[] = [
  {
    id: "rows",
    name: "Row-count band",
    desc: "scenario threshold vs baseline",
    weight: 32,
  },
  {
    id: "schema",
    name: "Schema check",
    desc: "declared compatibility rule",
    weight: 22,
  },
...
export const CHECKS_DE: readonly CheckDef[] = [
  {
    id: "rows",
    name: "Band für die Zeilenzahl",
    desc: "Szenarioschwelle gegen Basislinie",
    weight: 32,
  },
  {
    id: "schema",
    name: "Schemaprüfung",
    desc: "deklarierte Kompatibilitätsregel",
    weight: 22,
  },
  { id: "fresh", name: "Aktualität", desc: "Ziel des Datensatzes", weight: 24 },
  {
    id: "unique",
    name: "Eindeutigkeit",
    desc: "deklarierter Schlüssel und Granularität",
    weight: 22,
  },
];
...
export const CORRUPTIONS: Record<
  CorruptionId,
  { label: string; tripsBy: CheckDef["id"]; wrongVal: number }
> = {
  halfWrite: {
    label: "partial write (scenario retains 20% of rows)",
    tripsBy: "rows",
    wrongVal: 24.8,
  },
  schemaDrift: {
    label: "schema drift (new null column)",
    tripsBy: "schema",
    wrongVal: 0,
  },
  slaSlip: {
    label: "freshness miss (scenario partition arrives late)",
    tripsBy: "fresh",
    wrongVal: 142.3,
  },
  dupRows: {
    label: "duplicate rows (idempotency bug)",
    tripsBy: "unique",
    wrongVal: 284.6,
  },
};
...
export const CORRUPTIONS_DE: typeof CORRUPTIONS = {
  halfWrite: {
    label: "unvollständiger Schreibvorgang (Szenario behält 20% der Zeilen)",
    tripsBy: "rows",
    wrongVal: 24.8,
  },
  schemaDrift: {
    label: "Schemadrift (neue Spalte mit Nullwerten)",
    tripsBy: "schema",
    wrongVal: 0,
  },
  slaSlip: {
    label: "Aktualitätsziel verfehlt (Szenariopartition verspätet)",
    tripsBy: "fresh",
    wrongVal: 142.3,
  },
  dupRows: {
    label: "doppelte Zeilen (Idempotenzfehler)",
    tripsBy: "unique",
    wrongVal: 284.6,
  },
};
...
export function TrustMeterSim() {
  ...
  const activeWeight = Object.entries(checks)
    .filter(([, v]) => v)
    .reduce(
      (a, [k]) => a + (checkDefs.find((c) => c.id === k)?.weight ?? 0),
      0,
    );
  const trustPct = activeWeight;

  const toggle = (id: CheckDef["id"]) =>
  ...
  const run = () => {
    ...
    const step = () => {
      ...
      const corruption =
        isCorruptDay && corruptNow ? corruptions[corruptNow] : null;
      const trippingCheck = corruption?.tripsBy ?? null;
      const caught =
        isCorruptDay && !!trippingCheck && checksNow[trippingCheck];
      ...
      setResults((r) => [
        ...r,
        {
          day: d,
          ok,
          trippedBy: isCorruptDay ? trippingCheck : null,
          corrupt: isCorruptDay,
          caught,
        },
      ]);
      if (isCorruptDay && corruption && trippingCheck) {
        if (caught) {
          ...
          setOncallTicket({
            ...
            reason:
              corruption.label +
              ` · ${text("caught by", "erkannt durch")} ` +
              (checkDefs.find((c) => c.id === trippingCheck)?.name ?? ""),
          });
        } else {
  ...
  return (
    <Panel
      eyebrow={text(
        "live simulator · data-quality gates",
        "Live-Simulator · Datenqualitätsschranken",
      )}
    ...
    >
      <div className="tm-layout">
        <div className="tm-checks">
          <div className="tm-title">
            {text("Active checks", "Aktive Prüfungen")}
          </div>
          {checkDefs.map((c) => (
            <label
              key={c.id}
              className={`tm-check ${checks[c.id] ? "on" : ""}`}
            >
              <input
                type="checkbox"
                checked={checks[c.id]}
        ...
        <div className="tm-meter">
          <div className="tm-score-lab">
            {text("enabled check weight", "Gewicht aktiver Prüfungen")}
          </div>
          ...
          <div
            className={`tm-verdict ${trustPct >= 80 ? "ok" : trustPct >= 50 ? "warn" : "bad"}`}
          >
            {trustPct >= 80
              ? text(
                  "all modeled categories enabled",
                  "alle modellierten Kategorien aktiv",
                )
              : trustPct >= 50
                ? text("partial modeled coverage", "teilweise Modellabdeckung")
                : text(
                    "few modeled categories enabled",
                    "wenige modellierte Kategorien aktiv",
                  )}
          </div>
          <div className="tm-bar">
            <div className="tm-bar-track">
              <div
                className="tm-bar-zone bad"
                style={{ left: "0%", width: "50%" }}
              />
              <div
                className="tm-bar-zone warn"
                style={{ left: "50%", width: "30%" }}
              />
              <div
                className="tm-bar-zone ok"
                style={{ left: "80%", width: "20%" }}
              />
              <div
                className="tm-bar-needle"
                style={{ left: `${needle}%`, background: needleColor }}
              />
            </div>
          ...
          <div className="tm-breakdown">
            {checkDefs.map((c) => (
              <div
                key={c.id}
                className={`tm-bd ${checks[c.id] ? "on" : "off"}`}
              >
                <span className="tm-bd-dot" />
                <span className="tm-bd-n">{c.name}</span>
                <span className="tm-bd-w">
                  {checks[c.id] ? `+${c.weight}` : "-"}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="tm-impact">
        <div className="tm-impact-head">
          <div className="tm-impact-eyebrow">
            {text(
              "downstream dashboard · what the analyst sees",
              "nachgelagertes Dashboard · sichtbares Ergebnis",
            )}
          </div>
          <div className="tm-impact-title">
            {text("Exec Dashboard", "Management-Dashboard")} · DAU · USA ·{" "}
            {text("7-day avg", "7-Tage-Mittel")}
          </div>
        </div>
        <div className="tm-impact-grid">
          <div className="tm-impact-cell tm-impact-expected">
            <div className="lab">
              {text("Scenario reference", "Szenarioreferenz")}
            </div>
            <div className="big">
              142.3<span>M</span>
            </div>
            <div className="sub">
              {text("if pipeline ran clean", "bei fehlerfreiem Pipeline-Lauf")}
            </div>
          </div>
          ...
          <div className={`tm-impact-cell tm-impact-actual is-${dashState}`}>
            <div className="lab">
              {text("Displayed scenario value", "Angezeigter Szenariowert")}
            </div>
            <div className="big">
            ...
            <div className="sub">
              {dashState === "wrong" &&
                (() => {
                  const pct = Math.abs(
                    ((dashNumber - 142.3) / 142.3) * 100,
                  ).toFixed(0);
                  const diff = Math.round(Math.abs(dashNumber - 142.3));
                  const dir =
                    dashNumber > 142.3
                      ? `${diff}M ${text("extra", "zu viel")}`
                      : `${diff}M ${text("missing", "fehlend")}`;
                  return `${text("wrong by", "Abweichung")} ${pct}% · ${dir}`;
                })()}
              {dashState === "stale" &&
                text(
                  "3-day-old data · signal blocked upstream",
                  "3 Tage alte Daten · Signal upstream blockiert",
                )}
              {dashState === "ok" &&
                status === "ok" &&
                text(
        ...
        {oncallTicket && (
          <div className="tm-ticket">
            <div className="ti-k">
              {text("SIMULATED ALERT CREATED", "SIMULIERTER ALARM ERSTELLT")}
            </div>
            <div className="ti-id">#{oncallTicket.id}</div>
            <div className="ti-d">
              {text("day", "Tag")} {oncallTicket.day} · {oncallTicket.reason} ·{" "}
              {text("routed to", "zugewiesen an")} de_oncall
            </div>
          </div>
        )}
      </div>
      ...
      <div className="tm-timeline">
        <div className="tm-timeline-lab">
          {text("30-day run history", "Verlauf über 30 Tage")}
        </div>
        ...
        <div className="tm-timeline-legend">
          ...
          <span>
            <i className="sw corrupt-mark" />{" "}
            {text("corruption at day", "Fehler an Tag")} {CORRUPT_DAY + 1}
          </span>
        </div>
      </div>
      ...
      <div className="ctl-row">
        <div className="tm-corr-picker">
          <span className="tm-corr-lab">
            {text("Inject at day", "Fehler einfügen an Tag")} {CORRUPT_DAY + 1}:
          </span>
          <button
            type="button"
            className={`tm-corr ${!corrupt ? "on" : ""}`}
            onClick={() => setCorrupt(null)}
          >
            {text("none", "keiner")}
          </button>
          {(
            Object.entries(corruptions) as [
              CorruptionId,
              (typeof CORRUPTIONS)[CorruptionId],
            ][]
          ).map(([id, c]) => (
            <button
              type="button"
        ...
        >
          {status === "breach" && (
            <span style={{ color: "var(--theme-red)" }}>
              ✕{" "}
              {text(
                "Corrupt data shipped: gate was off",
                "Fehlerhafte Daten veröffentlicht: Schranke war deaktiviert",
              )}
            </span>
          )}
          {status === "stale" && (
          ...
          {status === "ok" && (
            <span style={{ color: "var(--theme-green)" }}>
              ✓ {text("Clean run", "Sauberer Lauf")} · 30/30
            </span>
          )}
        </div>
      </div>
