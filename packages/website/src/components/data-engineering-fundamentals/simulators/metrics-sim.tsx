...
export const QUESTIONS: readonly QuestionDef[] = [
  {
    q: "DAU in US last week?",
    metric: "dau",
    answer: "142.3M",
    src: "events_daily · 7-day avg · US",
  },
  {
    q: "Ad revenue yesterday?",
    metric: "revenue",
    answer: "$89.4M",
    src: "billable_impressions · 2026-04-19 · global",
  },
  {
    q: "Active creators this month?",
    metric: "creators",
    answer: "2.14M",
    src: "creator_posts_daily · 2026-04 MTD",
  },
];

export const QUESTIONS_DE: readonly QuestionDef[] = [
  {
    q: "DAU in den USA in der vergangenen Woche?",
    metric: "dau",
    answer: "142.3M",
    src: "events_daily · 7-Tage-Mittel · USA",
  },
  {
    q: "Werbeumsatz gestern?",
    metric: "revenue",
    answer: "$89.4M",
    src: "billable_impressions · 2026-04-19 · global",
  },
  {
    q: "Aktive Creator in diesem Monat?",
    metric: "creators",
    answer: "2.14M",
    src: "creator_posts_daily · 2026-04 MTD",
  },
...
function governedSteps(
  ...
  return [
    {
      k: "parse",
      title: text("Parse intent", "Absicht analysieren"),
      desc: text("question → metric lookup", "Frage → Metrik suchen"),
      ms: 500,
    },
    {
      k: "resolve",
      title: text("Resolve metric", "Metrik auflösen"),
      desc: `${text("Registry hit", "Treffer im Register")}: ${METRIC_REGISTRY[q2.metric].name}`,
      ms: 700,
    },
    {
      k: "bind",
      title: text("Bind grain & filters", "Granularität und Filter binden"),
      desc: text(
        "time window · cohort · geo",
        "Zeitfenster · Kohorte · Region",
      ),
      ms: 650,
    },
    {
      k: "compose",
      title: text("Compose SQL", "SQL zusammensetzen"),
      desc: text(
        "from metric formula + filters",
        "aus Metrikformel und Filtern",
      ),
      ms: 600,
    },
    {
      k: "execute",
      title: text("Execute on Presto", "Auf Presto ausführen"),
      desc: text("read declared source", "deklarierte Quelle lesen"),
      ms: 900,
    },
    {
      k: "answer",
      title: text(
        ...
        "Ergebnis und Kontext zurückgeben",
      ),
      desc: text(
        "record definition, filters, and source cutoff",
        "Definition, Filter und Quellenstichtag erfassen",
      ),
      ms: 400,
    },
  ];
  ...
  return [
    {
      ...
      title: text("Parse intent", "Absicht analysieren"),
      desc: text(
        'question → "find a relevant table"',
        'Frage → "passende Tabelle finden"',
      ),
      ms: 500,
    },
    {
      k: "search",
      title: text("Search warehouse schema", "Warehouse-Schema durchsuchen"),
      desc: text(
        "No registry: grep table names…",
        "Kein Register: Tabellennamen mit grep durchsuchen…",
      ),
      ms: 1000,
    },
    {
      k: "guess",
      title: text(
        "Pick table by name match",
        "Tabelle nach Namensähnlichkeit wählen",
      ),
      desc: `${text("Chose", "Gewählt")}: "dau_v3_deprecated_2021"`,
      ms: 800,
    },
    {
      k: "wrongcol",
      title: text(
        "Reference columns from memory",
        "Spalten aus dem Gedächtnis verwenden",
      ),
      desc: `${text("Wrote", "Geschrieben")}: user_cnt · daily_cnt`,
      ms: 900,
      error: true,
    },
    {
      k: "execute",
      title: text("Execute on Presto", "Auf Presto ausführen"),
      desc: text("Column not found: abort", "Spalte nicht gefunden: Abbruch"),
      ms: 500,
      error: true,
    },
  ];
...
export function MetricsSim() {
          ...
          const t2 = setTimeout(() => {
            ...
            setResult(
              errored
                ? {
                    err: true,
                    v: text("Query failed", "Abfrage fehlgeschlagen"),
                    src: text(
                      'ERROR: Column "user_cnt" cannot be resolved. Table "dau_v3_deprecated_2021" was archived 2022-08.',
                      'FEHLER: Spalte "user_cnt" kann nicht aufgelöst werden. Tabelle "dau_v3_deprecated_2021" wurde 2022-08 archiviert.',
                    ),
                  }
  ...
  return (
    <Panel
      eyebrow={text(
        "live simulator · metrics query",
        "Live-Simulator · Metrikabfrage",
      )}
      title={text(
        "The same question: with and without a metrics layer",
        "Dieselbe Frage mit und ohne Metrikschicht",
      )}
      meta={
        governed
          ? text("registered path", "registrierter Pfad")
    ...
    >
      <div className="aa-question-row">
        <input
          aria-label={text("Metric question", "Metrikfrage")}
          className="aa-q-input"
          value={question}
          onChange={(e) => setQuestion(e.target.value)}
          list="aa-qs"
          placeholder={text("Ask about a metric…", "Frage zu einer Metrik…")}
        />
        <datalist id="aa-qs">
          {questions.map((x) => (
        ...
        >
          {run.status === "running"
            ? text("…running", "…läuft")
            : text("▶ Run query", "▶ Abfrage starten")}
        </button>
      </div>
            ...
            {run.log.length === 0 && (
              ...
              >
                {text(
                  ...
                  "wartet: ▶ Abfrage starten drücken",
                )}
              </div>
            )}
            {run.log.map((s, i) => {
        ...
        <div className="aa-stage-col dark">
          <div className="aa-yaml-head">
            <span>
              {governed
                ? text(
                    "generated · composed from registry",
                    "generiert · aus dem Register zusammengesetzt",
                  )
                : text(
                    "ad-hoc · written by a hurried analyst",
                    "Ad hoc · unter Zeitdruck geschrieben",
                  )}
            </span>
            <span>SQL</span>
          </div>
      ...
      {result && (
        <div className={`aa-answer ${result.err ? "err" : ""}`}>
          <div className="lab">
            {result.err
              ? text("Execution failed", "Ausführung fehlgeschlagen")
              : text("Answer", "Antwort")}
          </div>
