...
export function BackfillSim() {
  ...
  const [log, setLog] = useState<readonly LogEntry[]>([
    { t: "00:00", k: "info", m: "Scheduler idle. Press ▶ Run backfill to queue 7 partitions.", _seq: 0 },
  ]);
  ...
  const commitPartition = (pi: number, wasRetried: boolean, resolve: () => void) => {
    ...
    if (modeRef.current === "overwrite") {
      ...
      append({ t: "  •  ", k: "ok", m: `task.${baseDate(pi)} → COMMIT · rows=${baseRows} ${wasRetried ? "(retry OK)" : ""}` });
    } else {
      const doubled = wasRetried;
      const rows = doubled ? baseRows * 2 : baseRows;
      updatePart(pi, { status: doubled ? "doubled" : "success", rows, fill: 100 });
      if (wasRetried) {
        append({ t: "  •  ", k: "err", m: `task.${baseDate(pi)} → INSERT appended DOUBLED rows (${baseRows * 2}): no idempotency` });
      } else {
        append({ t: "  •  ", k: "ok", m: `task.${baseDate(pi)} → INSERT committed · rows=${baseRows}` });
      }
    }
    resolve();
  };
      ...
      while (active < concurrency && index < queue.length) {
        ...
        runPartition(pi).then(() => {
          ...
          if (index < queue.length) launch();
            ...
            setRunning(false);
            append({ t: timestampFrom(start), k: "ok", m: "✓ Backfill complete" });
          }
        });
      }
    };
  ...
  return (
    <Panel
      eyebrow="live simulator · retry semantics"
      ...
      <div className="readout-grid">
        <div className={`readout ${allDone ? (anyDoubled ? "danger" : "ok") : "blue"}`}>
          ...
          <div className="r-v" style={{ fontSize: 18, textTransform: "uppercase" }}>
            {allDone ? (anyDoubled ? "corrupt" : "clean") : running ? "running" : "idle"}
          </div>
          <div className="r-s">
            {parts.filter((p) => p.status === "success" || p.status === "doubled").length}/{PART_COUNT} committed
          </div>
        </div>
