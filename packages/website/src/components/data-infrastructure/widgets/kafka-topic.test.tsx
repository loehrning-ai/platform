...
describe("KafkaTopic", () => {
  it("renders the producer, 4 partitions, and 3 consumers", () => {
    const { container } = render(<KafkaTopic lessonId="di-streaming" cpId="kafka" />);
  ...
  it("falls back to a static summary, without crashing, when getContext('2d') returns null", () => {
    ...
    try {
      expect(() => render(<KafkaTopic lessonId="di-streaming" cpId="kafka" />)).not.toThrow();
      expect(screen.getByRole("img", { name: /producer routes messages by key/ })).toBeInTheDocument();
    } finally {
  ...
  it("sending 3 messages awards the checkpoint and updates the produced counter", () => {
    vi.useFakeTimers();
    const { unmount } = render(<KafkaTopic lessonId="di-streaming" cpId="kafka" />);
