...
const PICK_TEXT_DE: Record<CapPick, string> = {
  CP: "CP verweigert während einer Partition Schreibvorgänge auf der Minderheitsseite. Beispiele: HBase, MongoDB in der Standardkonfiguration und Spanner.",
  AP: "AP bedient beide Seiten weiter. Lesezugriffe können veraltet sein; der Zustand wird später abgeglichen. Beispiele: Cassandra, DynamoDB und Riak.",
  CA: "CA ist nur ohne echte Netzpartition sicher. Verteilte Systeme müssen bei einer Partition zwischen CP und AP entscheiden.",
};
...
export function CapTriangle({ lessonId, cpId }: CapTriangleProps): JSX.Element {
  ...
  const draw = useCallback(
    (now: number): boolean => {
      ...
      const edges: {
        a: keyof typeof VERTS;
        b: keyof typeof VERTS;
        label: CapPick;
      }[] = [
        { a: "C", b: "A", label: "CA" },
        { a: "C", b: "P", label: "CP" },
      ...
      if (splitT > 0.05) {
        ...
        ctx.fillText(
          locale === "de" ? "NETZTEILUNG" : "PARTITION",
          BASE_X - 30,
          56,
        );
      }
      ...
      return (
        Math.abs(target - splitTRef.current) > 0.002 ||
  ...
  const emitRequests = useCallback(() => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (splitRef.current) {
        if (pickRef.current === "CP")
          outcome = partitionedSide ? "reject" : "ok";
        else if (pickRef.current === "AP") outcome = "stale";
        else if (pickRef.current === "CA")
          outcome = partitionedSide ? "fail" : "ok";
      }
  ...
  const setPickAndText = useCallback(
    (p: CapPick) => {
      ...
      setPick(p);
      picksRef.current += 1;
      setStatusText(locale === "de" ? PICK_TEXT_DE[p] : PICK_TEXT[p]);
      emitRequests();
      if (picksRef.current >= 2) complete();
    },
    [complete, emitRequests, locale],
  );
      ...
      {contextUnavailable ? (
        ...
        <div ref={wrapRef} className="h-[320px] w-full">
          <canvas
            ...
            aria-label={
              locale === "de"
                ? "Interaktives CAP-Dreieck für den Zielkonflikt zwischen Konsistenz, Verfügbarkeit und Partitionstoleranz."
                : "Interactive CAP theorem triangle showing the trade-off between consistency, availability, and partition tolerance."
            }
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        {(["CP", "AP", "CA"] as const).map((p) => (
          ...
          >
            {p === "CP"
              ? locale === "de"
                ? "CP, konsistent und partitionstolerant"
        ...
        >
          {split
            ? locale === "de"
              ? "Netzpartition aufheben"
              : "✓ heal partition"
            : locale === "de"
              ? "Netzpartition auslösen"
              : "⚡ inject network split"}
        </button>
      </div>
