...
describe("StackFlow", () => {
  it("renders all six stack layers and a canvas", () => {
    ...
    for (const name of ["source", "log", "processing", "storage", "serving", "consume"]) {
      expect(screen.getByText(name)).toBeInTheDocument();
    }
    expect(screen.getByRole("img", { name: /Animated diagram of an event flowing/ })).toBeInTheDocument();
  });
  ...
  it("falls back to a static summary, without crashing, when getContext('2d') returns null", () => {
    ...
    try {
      expect(() => render(<StackFlow lessonId="di-mental-model" cpId="flow" />)).not.toThrow();
      expect(screen.getByRole("img", { name: /An event flows source/ })).toBeInTheDocument();
      expect(screen.queryByRole("img", { name: /Animated diagram/ })).not.toBeInTheDocument();
    } finally {
