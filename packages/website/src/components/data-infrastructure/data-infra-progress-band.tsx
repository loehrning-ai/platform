 ...
 */
export function DataInfraProgressBand({ lessons }: DataInfraProgressBandProps): JSX.Element {
  ...
  const overallPct = lessons.length > 0 ? Math.round((totalDone / lessons.length) * 100) : 0;
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {DATA_INFRA_TRACKS.map((track) => {
        ...
        const pct = trackLessons.length > 0 ? Math.round((done / trackLessons.length) * 100) : 0;
        return (
          <div key={track.id} className="border-2 border-border bg-card p-4">
            <p className="font-mono text-[10px] font-bold uppercase tracking-[0.1em] text-brand-orange">
              {track.label}
            </p>
            <p className="mt-1 text-[13px] font-semibold text-foreground">{track.title}</p>
            <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-border">
              <div className="h-full bg-brand-orange transition-[width]" style={{ width: `${pct}%` }} />
            </div>
            <p className="mt-1.5 font-mono text-[11px] text-muted-foreground">
              {done} / {trackLessons.length} · {pct}%
            </p>
          </div>
        );
      })}
      <div className="border-2 border-foreground bg-card p-4 sm:col-span-2 lg:col-span-4">
        ...
        <p className="mt-1 font-mono text-[20px] font-bold text-foreground">
          {totalDone} / {lessons.length} lessons · {overallPct}%
        </p>
      </div>
    </div>
  );
