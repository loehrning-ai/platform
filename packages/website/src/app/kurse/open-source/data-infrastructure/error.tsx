...
export default function DataInfrastructureCourseError({
  error,
  reset,
}: {
  ...
  const copy = getDataInfraCourseCopy(locale).error;

  useEffect(() => {
    reportClientBoundaryError("data-infrastructure-course", error);
  }, [error]);
    ...
    <main className="flex min-h-[70svh] min-w-0 items-center justify-center bg-background px-4 py-16 sm:px-6">
      <div className="w-full max-w-lg min-w-0 border-2 border-foreground bg-card p-6 shadow-[6px_6px_0_var(--color-foreground)] sm:p-8">
        <p className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-destructive">
          Data Infrastructure / {locale === "de" ? "Fehler" : "error"}
        </p>
        ...
        <div className="mt-7 flex flex-col items-start gap-4 sm:flex-row sm:items-center">
          ...
          <Link
            href={technicalCourseHref("data-infrastructure", locale, { kind: "reader" })}
            className="inline-flex min-h-11 max-w-full items-center gap-2 break-words text-sm text-muted-foreground hover:text-foreground"
          >
