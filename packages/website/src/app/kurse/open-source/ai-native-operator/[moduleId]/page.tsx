...
export default async function AiNativeOperatorModulePage({
  ...
  return (
    <section className="mx-auto w-full max-w-[900px] overflow-x-clip px-4 py-12 sm:px-6">
      <div className="break-words font-mono text-xs text-muted-foreground">
        <Link
          href={courseHref(locale)}
          className="inline-flex min-h-11 min-w-11 items-center justify-center hover:text-foreground focus-visible:ring-2 focus-visible:ring-brand-orange focus-visible:ring-offset-2 focus-visible:ring-offset-background"
        >
          {copy.breadcrumb}
        </Link>
      ...
      <div className="mt-4 flex min-w-0 flex-col divide-y divide-border border-t border-border">
        {lessons.map((lesson) => (
          ...
          >
            <div className="font-mono text-xs text-muted-foreground sm:text-[12px]">
              {displayModuleNumber}.{lesson.lessonNumber}
            </div>
            <div
              aria-hidden="true"
              className="flex h-7 w-7 shrink-0 items-center justify-center border border-border font-mono text-xs font-bold text-muted-foreground"
            >
            ...
            <div className="col-start-3 font-mono text-xs text-muted-foreground sm:col-start-auto">
              {locale === "de"
      ...
      <div className="mt-10 flex items-center justify-end border-t border-border pt-6">
        {lessons[0] && (
          <Link
            ...
            className="inline-flex min-h-11 max-w-full items-center gap-2 break-words border-2 border-foreground bg-brand-orange px-5 py-2.5 text-[12px] font-bold uppercase tracking-wide text-white transition-colors hover:bg-foreground"
          >
            {copy.begin}
